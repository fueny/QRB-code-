...
请直接返回清理后的文本，不要添加任何解释或评论。"""),
        ...
        try:
            ...
            logger.info("文本块清理完成")
            logger.info(f"清理后的文本块: {cleaned_text[:100]}...")

            # 检查清理是否有效
            if cleaned_text == chunk:
                logger.warning("清理后的文本与原始文本相同，可能清理不成功")
    ...
    def clean_text(self, text: str) -> str:
        ...
        # 直接清理整个文本块，不使用工作流图
        try:
            cleaned_text = self._clean_chunk(text)
